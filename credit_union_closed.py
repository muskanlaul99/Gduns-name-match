...
options = Options()
prefs = {"download.default_directory": "/application/INFA/server/infa_shared/Scripts/FJ_GDUN_Automation/Ouput/",
...
driver.get(url)
time.sleep(5)
element2=driver.find_element(By.XPATH,"/html/body/div[4]/div[2]/div[4]/div[2]/div/ul/li[2]/button")
time.sleep(150)
element2.click()



#----------END COMMENTING TO STOP DOWNLOAD---------------------------------------------------------
...
time.sleep(15)
from zipfile import ZipFile
with ZipFile('/application/INFA/server/infa_shared/Scripts/FJ_GDUN_Automation/Output/CSV_ATTRIBUTES_CLOSED.zip', mode="r") as archive:
    archive.printdir()
    archive.extractall()
folder_path = '/application/INFA/server/infa_shared/Scripts/FJ_GDUN_Automation/Output'
...
df = pd.read_csv('/application/INFA/server/infa_shared/Scripts/FJ_GDUN_Automation/Output/CSV_ATTRIBUTES_CLOSED.CSV')
df['SOURCE']='CREDIT_UNION_CLOSED'
df=df[df.ID_NCUA!=0]
...
query = f"""TRUNCATE TABLE mstrstg.CREDIT_UNION_CLOSED"""
...
while i <len(temp):
    ...
    insert_query = f"""INSERT INTO mstrstg.CREDIT_UNION_CLOSED (ID_RSSD,NCUA_ID,NAME,COMPRESSED_NAME,CITY,COMPRESSED_CITY,STATE,SOURCE) VALUES {valu}"""
